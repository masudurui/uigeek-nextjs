...
import { Frost } from "@/components/canvasui/Frost"; // adjust path if needed
import { MetalFx } from "metal-fx"; // or "@/components/ui/metal-fx"
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        key={current}
        initial={{ scale: 0.85, rotateX: 10, opacity: 0.6 }}
      ...
      >
        <Image
          src={images[current]}
          alt={alts[current] || "Project screenshot"}
          fill
      ...
      <button
        onClick={onClose}
        className="absolute top-5 right-5 md:top-8 md:right-8 w-11 h-11 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-colors text-xl z-20"
        aria-label="Close"
      >
        ✕
      </button>
      ...
      {images.length > 1 && (
        <button
          ...
          className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-colors text-2xl z-20"
      ...
      {images.length > 1 && (
        <button
          ...
          className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-colors text-2xl z-20"
      ...
      >
        <div className="space-y-4 mb-6">
          {project.images.map((src, idx) => {
            ...
            const alt = project.imageAlts[idx] || `${project.title} screenshot`;

            return (
              <motion.div
                key={idx}
                className="relative w-full overflow-hidden rounded-lg md:rounded-xl lg:rounded-2xl border border-white/5 aspect-video cursor-zoom-in"
        ...
        <h2 className={`${sourceSerif.className} text-xl md:text-2xl font-semibold mb-3`}>
          {project.title}
        </h2>
        <p className="text-zinc-300 leading-relaxed mb-4 max-w-xl">{project.description}</p>
...
export default function Home() {
  return (
    <main className={`min-h-screen bg-[#0c0c0e] ${sourceSerif.variable}`}>
      <div className="max-w-[1600px] mx-auto grid md:grid-cols-[2fr_3fr]">
        ...
        >
          ...
          <h1 className={`${sourceSerif.className} text-2xl md:text-3xl lg:text-4xl font-bold leading-[1.05] tracking-tight mb-8`}>
            Product design agency from the Internet. 
          </h1>
          ...
          >
            <a
              ...
              className="inline-flex items-center justify-center gap-3 bg-[#1a1a1a] hover:bg-[#222] text-white px-8 py-4 rounded-xl font-medium transition-colors"
            >
              Let&apos;s Talk <span className="opacity-70">→</span> 1 Spot Available
            </a>
          </MetalFx>

          <div className="mt-auto pt-8 border-t border-white/5">
            <p className="text-xs text-white uppercase tracking-widest mb-4">Previously worked with</p>
        ...
        {/* Right Scrollable Content with Frost effect */}
              ...
              >
                <div className="flex flex-col gap-6 max-w-4xl lg:grid lg:grid-cols-2 lg:gap-6">
                  ...
                  <div className="rounded-3xl md:rounded-2xl border border-emerald-500/40 bg-emerald-950/80 p-8 flex flex-col text-white">
                    <div>
                      <h3 className={`${sourceSerif.className} text-xl font-semibold mb-2`}>Basic</h3>
                  ...
                  <div className="rounded-3xl md:rounded-2xl border border-white/10 bg-white p-8 flex flex-col text-black relative">
                    ...
                    <div>
                      <h3 className={`${sourceSerif.className} text-xl font-semibold mb-2`}>Launch</h3>
